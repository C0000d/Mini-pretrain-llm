...
def paragraph_length_filter(x):
    """Return false if a page has too few or too short lines"""
...
def paragraph_repetition_filter(x):
    """
    Returns False if a text has too many dupliated paragraphs
...
# get the text column only (different from Fine-tuning's instruction-style dataset)
pretraining_dataset = pretraining_dataset.select_columns(["text"]) # return a full dataset object
# Note: pretraining_dataset["text"] will only return a raw list of text

# 2. Pack open-sourced data
# here I scraped python script and translate them into hugging face dataset object
code_dir = "./code_dataset"  # path to store python scripts, should be existed before writing
